...
start_service() {
    ...
    if command_exists pm2 && [ "$1" != "--no-pm2" ]; then
        ...
        # 直接使用pm2启动，避免循环调用
        pm2 start "$APP_DIR/src/app.js" --name "claude-relay" --node-args="--expose-gc --initial-old-space-size=128 --max-old-space-size=256" --log "$APP_DIR/logs/pm2.log" 2>/dev/null
        sleep 2
...
start_service_direct() {
    ...
    if command_exists setsid; then
        # setsid方式（推荐）
        setsid nohup node --expose-gc --initial-old-space-size=128 --max-old-space-size=256 "$APP_DIR/src/app.js" > "$APP_DIR/logs/service.log" 2>&1 < /dev/null &
        local pid=$!
        sleep 1
        ...
        # 备用方式：使用nohup和disown
        nohup node --expose-gc --initial-old-space-size=128 --max-old-space-size=256 "$APP_DIR/src/app.js" > "$APP_DIR/logs/service.log" 2>&1 < /dev/null &
        local pid=$!
