...
class SystemIntegrationTester {
  ...
  async captureSystemStats() {
    try {
      ...
      return {
        ...
        connectionPools: connectionPoolResponse.status === 'fulfilled' ? connectionPoolResponse.value.data : null,
    ...
    for (let i = 0; i < testMessages.length; i++) {
      ...
      try {
        ...
        if (response.status === 200 && response.data.content) {
          ...
          logger.success(`✅ 端到端测试 ${i + 1}: ${totalLatency.toFixed(2)}ms - ${response.data.content[0]?.text?.slice(0, 50)}...`)
        } else {
  ...
  async testPerformanceImprovement() {
    ...
    if (latencies.length > 0) {
      ...
      logger.info(`📊 性能测试结果: 平均 ${avgLatency.toFixed(2)}ms, 范围 ${minLatency.toFixed(2)}-${maxLatency.toFixed(2)}ms`)
    }
  }
    ...
    for (const test of compatibilityTests) {
      try {
        ...
        if (success) {
          ...
          logger.error(`❌ 兼容性测试 - ${test.name}: 期望 ${test.expectedStatus}, 实际 ${response.status}`)
        }
  ...
  async testResourceUsage() {
    ...
    try {
      ...
      for (let i = 0; i < requestCount; i++) {
        promises.push(
          axios.post(`${this.baseUrl}/api/v1/messages`, {
      ...
      this.testResults.resourceUsageTests.push({
        ...
        memoryIncrease: afterStats.memory ? 
          ((afterStats.memory.heapUsed - beforeStats.memory.heapUsed) / 1024 / 1024).toFixed(2) : 'N/A',
        timestamp: new Date().toISOString()
      })
      
      logger.info(`📊 资源使用测试完成: ${successCount}/${requestCount} 成功, 吞吐量: ${(successCount / (totalTime / 1000)).toFixed(2)} req/s`)
  ...
  analyzeEndToEndResults() {
    ...
    const avgLatency = successResults.reduce((sum, r) => sum + r.totalLatency, 0) / successResults.length
  ...
  printIntegrationReport(report) {
    ...
    logger.info(`🌐 端到端测试: ${report.analysis.endToEnd.status} (成功率: ${report.analysis.endToEnd.successRate}%)`)
    logger.info(`⚡ 性能测试: ${report.analysis.performance.status} (平均延迟: ${report.analysis.performance.avgLatency}ms)`)
    logger.info(`🔄 兼容性测试: ${report.analysis.compatibility.status} (成功率: ${report.analysis.compatibility.successRate}%)`)
    logger.info(`🛡️ 错误恢复测试: ${report.analysis.errorRecovery.status} (成功率: ${report.analysis.errorRecovery.successRate}%)`)
    logger.info(`📊 资源使用测试: ${report.analysis.resourceUsage.status} (吞吐量: ${report.analysis.resourceUsage.throughput} req/s)`)
...
if (require.main === module) {
  ...
  tester.runFullIntegrationTest()
    .then((report) => {
      ...
      // 根据整体评估决定退出码
      if (report.overallAssessment.status === 'excellent' || report.overallAssessment.status === 'good') {
        process.exit(0)
      } else {
        process.exit(1)
      }
