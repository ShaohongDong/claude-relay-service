...
class ConnectionPoolTester {
    ...
    for (const account of this.testAccounts) {
      try {
        ...
        for (let i = 0; i < testConcurrency; i++) {
          promises.push(
            this.performConcurrentConnectionTest(account, testDuration)
              .then((result) => {
                if (result.success) {
                  successCount++
        ...
        logger.info(
          `⚡ 性能测试完成 - ${account.name}: ${successCount}成功/${errorCount}失败, 平均延迟: ${avgLatency.toFixed(2)}ms`
        )
      } catch (error) {
  ...
  async testSystemStability() {
    ...
    logger.info(
      `🛡️ 稳定性测试完成: ${successOperations}/${totalOperations} (${successRate.toFixed(2)}%成功率)`
    )
  ...
  analyzeReconnectionResults() {
    ...
    const avgReconnectionTime =
      successResults.reduce((sum, r) => sum + r.reconnectionTime, 0) / successResults.length
    const maxReconnectionTime = Math.max(...successResults.map((r) => r.reconnectionTime))

    return {
      status:
        avgReconnectionTime <= 3000 ? 'excellent' : avgReconnectionTime <= 5000 ? 'good' : 'poor',
  ...
  analyzePerformanceResults() {
    ...
    const avgThroughput =
      results.reduce((sum, r) => sum + parseFloat(r.throughput), 0) / results.length
  ...
  async runAllTests() {
    try {
      ...
      logger.info(
        `📊 预热测试: ${report.preheatAnalysis.status} (平均延迟: ${report.preheatAnalysis.avgLatency}ms)`
      )
      logger.info(
        `🔄 重连测试: ${report.reconnectionAnalysis.status} (平均重连时间: ${report.reconnectionAnalysis.avgReconnectionTime}ms)`
      )
      logger.info(
        `⚡ 性能测试: ${report.performanceAnalysis.status} (错误率: ${report.performanceAnalysis.errorRate}%)`
      )
      logger.info(
        `🛡️ 稳定性测试: ${report.stabilityAnalysis.status} (成功率: ${report.stabilityAnalysis.successRate}%)`
      )
