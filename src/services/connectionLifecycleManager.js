...
class ConnectionLifecycleManager extends EventEmitter {
  constructor() {
    ...
    // 生命周期配置
    this.config = {
      maxConnectionAge: 30 * 60 * 1000, // 30分钟最大连接寿命
      healthCheckInterval: 2 * 60 * 1000, // 2分钟兜底健康检查
      connectionRotationInterval: 15 * 60 * 1000, // 15分钟连接轮换检查
      inactiveConnectionThreshold: 10 * 60 * 1000, // 10分钟非活跃连接阈值
      memoryCleanupInterval: 5 * 60 * 1000, // 5分钟内存清理检查
      performanceAnalysisInterval: 60 * 1000, // 1分钟性能分析
  ...
  registerConnection(accountId, connectionId, connectionData) {
    ...
    logger.debug(`♻️ 连接已注册: ${connectionId} (账户: ${accountId})`)
  ...
  unregisterConnection(connectionId, reason = 'manual') {
    ...
    logger.info(`♻️ 连接已注销: ${connectionId} (账户: ${accountId}, 原因: ${reason}, 寿命: ${lifetime}ms)`)
  ...
  checkConnectionLimit(accountId) {
    ...
    if (connectionCount > this.config.maxConnectionsPerAccount) {
      logger.warn(`⚠️ 账户连接数超限: ${accountId} (${connectionCount}/${this.config.maxConnectionsPerAccount})`)
  ...
  async performFallbackHealthCheck() {
    ...
    logger.info(`🏥 兜底健康检查完成: ${healthyCount}健康/${checkedCount}总计 (${unhealthyCount}不健康)`)
  ...
  async checkConnectionHealth(connection) {
    ...
    if (age > this.config.maxConnectionAge) {
      logger.debug(`♻️ 连接超龄: ${connection.connectionId} (${age}ms > ${this.config.maxConnectionAge}ms)`)
      return false
    }
    ...
    if (connection.usageCount > 0) {
      ...
      if (errorRate > 0.2) { // 20%错误率阈值
        logger.debug(`♻️ 连接错误率高: ${connection.connectionId} (${(errorRate * 100).toFixed(2)}%)`)
        return false
      }
    }
    
    // 检查Socket状态（如果可访问）
    if (connection.agent && connection.agent.sockets) {
    ...
    try {
      ...
      for (const hostSockets of Object.values(sockets)) {
        if (Array.isArray(hostSockets)) {
          ...
          activeSockets += hostSockets.filter(socket => 
            socket.readyState === 'open' || socket.readyState === 'opening'
          ).length
        }
      }
  ...
  generatePerformanceReport() {
    ...
    if (connections.length > 0) {
      ...
      const latencies = connections.filter(conn => conn.averageLatency).map(conn => conn.averageLatency)
      ...
      if (latencies.length > 0) {
        report.performanceMetrics.averageLatency = latencies.reduce((sum, lat) => sum + lat, 0) / latencies.length
      }
    }
  ...
  checkPerformanceIssues(analysis) {
    ...
    if (performanceMetrics.averageAge > this.config.maxConnectionAge * 0.8) {
      logger.warn(`📊 连接普遍老化: 平均寿命 ${(performanceMetrics.averageAge / 1000 / 60).toFixed(2)}分钟`)
      this.emit('performance:issue', {
