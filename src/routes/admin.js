...
// 获取连接池统计信息
router.get('/connection-pools/stats', authenticateAdmin, async (req, res) => {
  try {
...
// 清理连接池
router.post('/connection-pools/cleanup', authenticateAdmin, async (req, res) => {
  try {
    ...
    logger.error('Failed to cleanup connection pools:', error)
    res.status(500).json({
      success: false,
      message: 'Failed to cleanup connection pools',
      error: error.message
    })
  }
})
...
// 获取连接池详细状态（调试用）
router.get('/connection-pools/debug', authenticateAdmin, async (req, res) => {
  try {
    const stats = ProxyHelper.getConnectionPoolStats()
    ...
    logger.error('Failed to get connection pool debug info:', error)
    res.status(500).json({
      success: false,
      message: 'Failed to get connection pool debug information',
      error: error.message
    })
  }
})
